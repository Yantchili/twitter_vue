...
<template>
    ...
    <div class="row">
      <div class="container mt-5 col-5 text-center">
        ...
        <div class="row row-cols-1">
          <allPostList v-for="post in displayedPosts" :key="post.id" :post="post" @addLike="toggleLike($event)"
            @usernameClicked="getUserPage" />
        </div>
      </div>
    </div>
  </div>
  <div class="btn-group col-md-2 offset-md-5">
    ...
    <button type="button" class="btn btn-sm btn-outline-secondary" v-for="pageNumber in pages.slice(page - 1, page + 5)"
      @click="page = pageNumber" :key="pageNumber">{{
        pageNumber
      }}</button>
...
export default ({
  ...
  setup() {
    ...
    fetchAllPosts()
    const setPostNumber = () => {
      numOfPages = Math.ceil(AllPosts.value.length / perPage.value)
    ...
    const displayedPosts = computed(() => {
      return paginate(AllPosts.value);

    });
    watch(() => AllPosts.value, () => {
      setPostNumber()
    });
    ...
    let searchVal = ""
    return {
      AllPosts, pages, paginate, displayedPosts, page, setPostNumber, toggleLike, users, getLikeList, isLiked, getUserPage, searchVal, toSearchPage
    }
